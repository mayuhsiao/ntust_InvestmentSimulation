...
| 💹 **交易下單** | 台股上市櫃 12,000+ 檔以當日收盤價買賣（支援零股），**美股**輸入英文代號即可交易，自動以當日匯率換算台幣 |
...
- **一律以收盤價成交，不能指定價格**
- **盤中不開放下單**：平日 09:00–14:00（13:30 收盤、約 14:00 資料定案）一律鎖住。
  否則同學看著即時走勢、卻用昨天的收盤價下單，等於穩賺不賠
- 平日 14:00 之後到隔天 09:00 前，用**當日收盤價**交易
- **六日與休市日全天開放**，以最近一個交易日（例如週五）的收盤價成交
- 老師不受時段限制，方便上課示範與補單；也可在競賽設定改成「不限時段」

> 國定假日若剛好落在平日，系統仍會在 09:00–14:00 鎖住下單（無法自動判斷休市）。
> 需要的話，老師可暫時把「交易時段」改成「不限時段」。

### 美股

- 搜尋框輸入**英文代號或公司名**（`AAPL`、`nvidia`、`TSLA`）即可交易，涵蓋 NASDAQ／NYSE／NYSEArca／BATS 掛牌的股票與 ETF
- 美股以**股**為單位（沒有「張」）
- 所有金額都用**當日 USD/TWD 匯率換算成新台幣**，與台股共用同一個帳戶與排行榜。
  匯率取自 `TWD=X` 的日收盤，和股價一樣逐日對應
- 因此**報酬率會同時反映股價與匯率變動** —— 這正是實際買美股會遇到的狀況，也可以在課堂上當作匯率風險的例子
- 畫面上股價顯示原幣（`$326.57`），市值、損益、排名一律是台幣

### 交易成本（可在競賽設定調整）

| 項目 | 預設 | 說明 |
|---|---|---|
| 台股手續費 | 0.1425% | 買賣都收，無條件捨去到元 |
